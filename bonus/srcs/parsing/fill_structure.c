...
static int	set_world(t_obj_list *obj_list, char *line, size_t *obj_index, size_t *light_index)
{
	...
	if (!ft_strcmp(*splitted_line, "sp"))
		ret = new_sphere(&obj_list->obj[(*obj_index)++], splitted_line);
	else if (!ft_strcmp(*splitted_line, "pl"))
		ret = new_plan(&obj_list->obj[(*obj_index)++], splitted_line);
	else if (!ft_strcmp(*splitted_line, "cy"))
		ret = new_cylinder(&obj_list->obj[(*obj_index)++], splitted_line);
		...
		ret = new_light(&obj_list->light[(*light_index)++], splitted_line);
...
int	fill_structure(t_world *world, const char *filename)
{
	...
	while (1)
	{
		...
		if (!line)
			break ;
		if (error == false && set_world(&world->obj_list, line, &obj_index, &light_index))
			error = true;
		free(line);
	}
